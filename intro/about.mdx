...
|---------|-----------|-------------|
| **Channels** | WebSocket + REST publish | Real-time pub/sub fan-out |
| **Queues** | REST | Durable jobs, delayed/scheduled work |

## How it fits together

1. A **user** signs into QPub Cloud and belongs to an **account**.
...
QPub ships official clients for JavaScript/TypeScript, React, and Go. All expose the same surfaces:

- `QPub.Socket` — WebSocket client (subscribe + publish)
- `QPub.Rest` — HTTP client (publish, tokens, queues)

See [Quickstart](/getting-started/quickstart.mdx) and [SDKs](/getting-started/sdks.mdx).
...
- Cloud login (email/password or OAuth) is separate from API key / JWT auth.
