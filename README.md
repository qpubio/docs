...
9. Multi-example pages: wrap in `<ExampleScope groups={["Socket","REST"]} languages={["javascript","react"]}>` (props are also parsed server-side so chrome SSR without a layout jump). **API** / **Language** controls appear in docs chrome (right rail on desktop; top of the docs nav drawer on mobile), not in the MDX body — refer to them that way in prose, not as “Examples”. Author switched samples as fenced children (MDX strips indent inside template-literal props): `<CodeExamples group="Socket">` + ` ```javascript ` / ` ```react ` fences, then `</CodeExamples>`. Use `<When group lang>` for variant prose — inactive branches stay in the HTML (hidden) so Socket and REST are both indexable. See `getting-started/quickstart.mdx`.
