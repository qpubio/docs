...
10. Multi-example pages: wrap in `<ExampleScope groups={["Socket","REST"]} languages={["javascript","react","go"]}>` (props are also parsed server-side so chrome SSR without a layout jump). **API** / **Language** controls live in docs chrome — right rail on `lg+`, above the left sidebar on `md`–`lg`, and inside the bottom docs navigation drawer on mobile — not in the MDX body. In prose, point readers there (not “Examples”). Author switched samples as fenced children (MDX strips indent inside template-literal props): `<CodeExamples group="Socket">` + ` ```javascript ` / ` ```react ` / ` ```go ` fences, then `</CodeExamples>`. Use `<When group lang>` for variant prose (`javascript`, `react`, `go`) — inactive branches stay in the HTML (hidden) so Socket and REST are both indexable. Go samples must match [qpub-go](https://pkg.go.dev/github.com/qpubio/qpub-go) v0.2.0+ API (`import "github.com/qpubio/qpub-go"`). See `getting-started/quickstart.mdx`.
