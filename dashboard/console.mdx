...
You need at least one [API key](/docs/dashboard/api-keys) in the project before the console can connect.
...
| Panel | Purpose |
|-------|---------|
| **Controls** | Main connection: pick an API key, choose a channel, publish |
...
- Use separate API keys with scoped [permissions](/docs/core/auth/permissions) when testing access control.
- Prefer the console for development and debugging—not for production traffic.
- For production apps, use [`@qpub/sdk`](/docs/getting-started/sdks) or the [REST](/docs/api/rest-api) / [Socket](/docs/api/socket-protocol) APIs.
