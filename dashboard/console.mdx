...
You need at least one [API key](/dashboard/api-keys) in the project before the console can connect.
...
- Use separate API keys with scoped [permissions](/core/auth/permissions) when testing access control.
- Prefer the console for development and debugging—not for production traffic.
- For production apps, use [`@qpub/sdk`](/getting-started/sdks) or the [REST](/api/rest-api) / [Socket](/api/socket-protocol) APIs.
