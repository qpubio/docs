...
Permissions control which actions a credential may perform on which resources (usually channel or queue names).
...
TODO: Document pattern wildcards (for example `flights.*`) only after they are verified in the permission matcher. Prefer exact names or `"*"` today.
