...
<When lang="react">
Prefer a JWT or [token request](/core/auth/token) in `SocketProvider` for browsers instead of a production API key secret.
</When>
...
- Message size and rate limits depend on your [plan](/pricing/limits).
- Subscribers need a [Socket subscription](/core/pubsub/subscribe) to receive messages in realtime.
