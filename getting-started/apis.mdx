...
Details: [Authentication](/docs/core/auth).
...
Full list: [REST API](/docs/api/rest-api).

## WebSocket capabilities

Connect to `GET /v1` with auth credentials, then exchange JSON messages with numeric `action` codes (subscribe, publish, ping, …).

Full protocol: [Socket Protocol](/docs/api/socket-protocol).

## Dashboard API

The dashboard uses a separate user-auth API (email/password or OAuth) for accounts, projects, API keys, usage, and billing. It is not used by Socket/REST messaging clients. See [Dashboard](/docs/dashboard).
